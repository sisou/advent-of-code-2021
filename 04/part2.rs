fn main() {
    ...
    for draw in draws {
        let mut boards_that_bingoed = vec![];

        let num_boards_remaining = boards.len();

        for (i, board) in boards.iter_mut().enumerate() {
            board.call(draw);

            if board.has_bingo() {
                if num_boards_remaining == 1 {
                    let unmarked_sum = boards[0].unmarked_sum();
                    println!("Last board to have bingo had score: {}", unmarked_sum * draw);
                    return;
                }

                boards_that_bingoed.push(i);
            }
        }

        for (k, i) in boards_that_bingoed.iter().enumerate() {
            boards.remove(i - k);
        }
    }
}
